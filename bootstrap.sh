...
# Fill these in before running start-java-edot.sh or start-full-edot.sh.
#
# Find your values in Kibana:
#   Observability → Add data → APM → OpenTelemetry
#   or: Stack Management → API Keys (create one with APM permissions)
...
EOF
    info ".env.otel template created — fill in your Elastic credentials before instrumenting"
fi
